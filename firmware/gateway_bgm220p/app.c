/***************************************************************************//**
 * @file app_scanner.c
 * @brief EAD Scanner Decrypting Custom Health Data Payload
 ******************************************************************************/
#include "sl_common.h"
#include "sl_bt_api.h"
...
// Lasting connection control variables
  ...
  while (i < adv_report->data.len) {
    ...
    if (ad_type == SL_BT_ENCRYPTED_DATA_AD_TYPE) {
      ...
      if (sc != 0) {
        ...
        // advertisement_info.ad_data holds: [0]=len, [1]=AD type (0x16), [2..]=health_data_t
        ...
        app_log("  - Body Temperature:  %d C\r\n", received_health->body_temp);
