...
#define PPG_WINDOW_SIZE   125   
#define INFERENCE_HOP     25    // chay lai HR/SpO2 + AI moi 25 mau moi (~1s)

#define META_HR_MEAN     80.0f   
#define META_HR_STD      15.0f   
#define META_SPO2_MEAN   97.0f   
#define META_SPO2_STD     2.0f   
...
extern "C" void ml_init(void)
{
    ...
    for (int i = 0; i < 4; i++) {   
        TfLiteTensor *t = sl_ml_bp_model_int8_model_handle.input_tensor(i);
        if (t == NULL) {
            app_log_info("[ML][DEBUG] input_tensor(%d) = NULL (het input)\r\n", i);
            break;
        }
        app_log_info("[ML][DEBUG] input_tensor(%d): dims->size=%d, dims=[",
                      i, t->dims->size);
...
static bool find_input_tensors(TfLiteTensor **out_wave, TfLiteTensor **out_meta)
{
    TfLiteTensor *t0 = sl_ml_bp_model_int8_model_handle.input_tensor(0);
    TfLiteTensor *t1 = sl_ml_bp_model_int8_model_handle.input_tensor(1);

    if (t0 == NULL) {
        app_log_error("[ML] input_tensor(0) = NULL - model khong co input nao ca\r\n");
        return false;
    }
    if (t1 == NULL) {
        app_log_error("[ML] input_tensor(1) = NULL - model chi co 1 INPUT, "
                       "chua phai ban 2-input (waveform+meta). Kiem tra lai "
                       "file .tflite da import vao project co dung la ban "
                       "moi xuat tu train_bp_model.py hay khong.\r\n");
        return false;
    }

    if (t0->dims->size == 3 && t1->dims->size == 2) {
        *out_wave = t0; *out_meta = t1;
    } else if (t1->dims->size == 3 && t0->dims->size == 2) {
        *out_wave = t1; *out_meta = t0;
    } else {
        app_log_error("[ML] Khong nhan dien duoc tensor waveform/meta theo dims\r\n");
        return false;
    }
    return true;
}
...
static void run_vitals_and_ai(void)
{
    ...
    if (!vit.finger_detected) {
        ...
        return;   // khong co ngon tay -> khong chay AI, tranh suy luan tren nhieu
    }

    float sbp = 0.0f, dbp = 0.0f;
    bool bp_ok = false;

    if (vit.hr_valid && vit.spo2_valid) {
        TfLiteTensor *wave_t = NULL;
        TfLiteTensor *meta_t = NULL;

        if (find_input_tensors(&wave_t, &meta_t)) {
            float w_scale = wave_t->params.scale;
            int32_t w_zp  = wave_t->params.zero_point;
            float m_scale = meta_t->params.scale;
            int32_t m_zp  = meta_t->params.zero_point;
            ...
            float hr_norm   = (vit.hr   - META_HR_MEAN)   / META_HR_STD;
            float spo2_norm = (vit.spo2 - META_SPO2_MEAN) / META_SPO2_STD;
            meta_t->data.int8[0] = quantize_i8(hr_norm,   m_scale, m_zp);
            meta_t->data.int8[1] = quantize_i8(spo2_norm, m_scale, m_zp);
    ...
    app_log_info("[RESULT]");
    if (vit.hr_valid)   app_log_info(" HR=%.1f bpm", vit.hr);
    else                app_log_info(" HR=--");
    if (vit.spo2_valid) app_log_info(" SpO2=%.1f%%", vit.spo2);
    else                app_log_info(" SpO2=--");
                                ...
                                uint32_t red_raw, uint32_t ir_raw)
{
    ...
    if (cb_fill_count < PPG_WINDOW_SIZE) {
        cb_fill_count++;
        if (cb_fill_count == PPG_WINDOW_SIZE) {
            app_log_info("[ML] Da du 5s du lieu, bat dau tinh toan...\r\n");
        }
    }
